...
class PointLoads(Base):
    ...
    __pLoadsNode__ = defaultdict(list)
    __pLoadsElement__ = defaultdict(list)
    id = Column('id', Integer, primary_key=True)
    user_id = Column('user_id', String(45), nullable=False)
    nn = Column('number', Integer)
    c = Column('c', Float)
    ...
    def __init__(self):
        if self.c == 99999:
            self.__class__.__pLoadsNode__[self.nn].append(self)
        else:
            self.__class__.__pLoadsElement__[self.nn].append(self)



class DistLoads(Base):
    __tablename__ = 'dist_loads'
    __distLoads__ = defaultdict(list)
    id = Column('id', Integer, primary_key=True)
    user_id = Column('user_id', String(45), nullable=False)
    en = Column('number', Integer)
    ...
    @reconstructor
    def __init__(self):
    	self.__class__.__distLoads__[self.en].append(self)
