...
def load_data():
	...
	elements_data = elements.query.all()
	...
	truss_elements = elements.query.filter_by(type='truss').all()
	truss_nodes = []
	for r in truss_elements:
		a = r.nodei
		
		b = r.nodej
		check1 = elements.query.filter_by(nodei=a, type='beam').all() + elements.query.filter_by(nodej=a, type='beam').all()
		
		check2 = elements.query.filter_by(nodej=b, type='beam').all() + elements.query.filter_by(nodei=b, type='beam').all()
		if len(check1)==0:
